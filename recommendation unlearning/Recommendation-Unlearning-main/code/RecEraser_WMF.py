...
class RecEraser_WMF(object):
    ...
    def _init_weights(self):
        ...
        all_weights['WA'] = tf.Variable(
            tf.truncated_normal(shape=[self.emb_dim, int(self.attention_size)], mean=0.0, stddev=tf.sqrt(
                tf.math.divide(2.0, int(self.attention_size) + self.emb_dim))), dtype=tf.float32, name='WA')
        all_weights['BA'] = tf.Variable(tf.constant(0.00, shape=[int(self.attention_size)]), name="BA")
        all_weights['HA'] = tf.Variable(tf.constant(0.01, shape=[int(self.attention_size), 1]), name="HA")

        # item attention
        all_weights['WB'] = tf.Variable(
            tf.truncated_normal(shape=[self.emb_dim, int(self.attention_size)], mean=0.0, stddev=tf.sqrt(
                tf.math.divide(2.0, int(self.attention_size) + self.emb_dim))), dtype=tf.float32, name='WB')
        all_weights['BB'] = tf.Variable(tf.constant(0.00, shape=[int(self.attention_size)]), name="BB")
        all_weights['HB'] = tf.Variable(tf.constant(0.01, shape=[int(self.attention_size), 1]), name="HB")
...
def get_lables(temp_set, k=0.9999):
    max_item = 0
    item_lenth = []
    for i in temp_set:#for each user in train_items
        item_lenth.append(len(temp_set[i]))#append(length of items of ith user)
        if len(temp_set[i]) > max_item:
            max_item = len(temp_set[i])#get max items of ith user
    item_lenth.sort()

    max_item = item_lenth[int(len(item_lenth) * k) - 1]#len(item_lenth):user_num*k
    print(max_item)
    for i in temp_set:
        if len(temp_set[i]) > max_item:
            temp_set[i] = temp_set[i][0:max_item]#get max_item items for each user
        while len(temp_set[i]) < max_item:#else add last item
...
if __name__ == '__main__':
    ...
    train_items = copy.deepcopy(data_generator.train_items)#[uid:items]
    ...
    for i in range(args.part_num):
        ...
        print("local_train_items[{{i:int}}]".format(i=i))
        local_train_lable.append(get_lables(local_train_items[i]))

    t0 = time()
    model = RecEraser_WMF(n_users+1, n_items+1,local_items)
