...
class Model(nn.Module):
    ...
    def __init__(self, info, generator, create_embeddings=True):
        super().__init__()
        assert isinstance(info, self.get_infotype())
        self.info = info
        self.embedding_size = info['embedding_size']
        self.embed_L2_norm = info['embed_L2_norm']
        self.num_users = generator.num_users
        self.num_bundles = generator.num_bundles
        self.num_items = generator.num_items
        self.part_num=generator.part_num
        if create_embeddings:
            # embeddings
            self.users_feature = nn.Parameter(
                torch.FloatTensor(self.num_users, self.part_num, self.embedding_size))
            nn.init.xavier_normal_(self.users_feature)
            self.bundles_feature = nn.Parameter(
                torch.FloatTensor(self.num_bundles, self.part_num, self.embedding_size))
